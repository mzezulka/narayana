...
public class Uid implements Cloneable, Serializable {
    ...
    /**
     * Create Uid from string representation. boolean arg says whether to give up if
     * an error is detected or to simply replace with nullUid.
     */
    ...
    public Uid(String uidString, boolean errsOk) {
        ...
        if (!_valid) {
            if (errsOk) {
                try {
                    /*
                     * We do this so the instance is printable, but it shouldn't be usable from here
                     * on in!
                     */
    ...
    public Uid(long[] addr, int processId, int time, int incr) {
        try {
            ...
            throw new FatalError(tsLogger.i18NLogger.get_common_Uid_11() + ex);
        }
    }
    ...
    public String stringForm() {
        ...
        if (_stringForm == null)
            _stringForm = Utility.longToHexString(hostAddr[0]) + Uid.breakChar + Utility.longToHexString(hostAddr[1])
                    + Uid.breakChar + Utility.intToHexString(process) + Uid.breakChar + Utility.intToHexString(sec)
                    + Uid.breakChar + Utility.intToHexString(other);

        return _stringForm;
    }

    /**
     * @return a string representation of the Uid with all : replaced by _ so that
     *         the name may be used as a file name.
     */

    public String fileStringForm() {
        return Utility.longToHexString(hostAddr[0]) + Uid.fileBreakChar + Utility.longToHexString(hostAddr[1])
                + Uid.fileBreakChar + Utility.intToHexString(process) + Uid.fileBreakChar + Utility.intToHexString(sec)
                + Uid.fileBreakChar + Utility.intToHexString(other);
    }
    ...
    public byte[] getBytes() {
        /*
         * We should only really be doing this once, so overhead should be negligible.
         */
    ...
    public boolean equals(Uid u) {
        ...
        return ((other == u.other) && (sec == u.sec) && (process == u.process) && (hostAddr[0] == u.hostAddr[0])
    ...
    /*
     * Serialization methods. Similar to buffer packing. If the Uid is invalid the
    ...
    /*
     * Serialization methods. Similar to buffer unpacking. If the unserialization
     * fails then an IOException is thrown.
     *
     * @since JTS 2.1.
     */
    ...
    /*
     * Is an idempotent operation, so can be called more than once without adverse
     * effect.
     */
    ...
    /*
     * Since we may be given a Uid from the file system (which uses '_' to separate
     * fields, we need to be able to convert.
     */
    ...
    private static final Uid MAX_UID = new Uid("7fffffffffffffff:7fffffffffffffff:7fffffff:7fffffff:7fffffff");

    private static final Uid MIN_UID = new Uid("-8000000000000000:-8000000000000000:-80000000:-80000000:-80000000");
