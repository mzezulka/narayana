...
public class StateManager {
	 ...
	 * memory resident form, or it may simply be a no-op. Full activation is only
	 * necessary if the object is currently marked as being PASSIVE (that is, the
	 * object was constructed as being of type ANDPERSISTENT with an existing uid
	 * and has not already been activated). Objects that are not of type
	 * ANDPERSISTENT or are persistent but have not yet been saved in an object
	...
	public synchronized boolean activate(String rootName) {
		if (tsLogger.logger.isTraceEnabled()) {
			tsLogger.logger.trace("StateManager::activate( " + ((rootName != null) ? rootName : "null")
		...
		/*
		 * Check if this action has logged its presence before. If not we force creation
		 * of an ActivationRecord so that each thread/action tree has an
		 * ActivationRecord in it. This allows us to passivate the object when the last
		 * thread has finished with it, i.e., when the last ActivationRecord is gone.
		 */
		...
		if ((action != null) && (action.status() == ActionStatus.RUNNING)) {
			/*
			 * Only check for top-level action. This is sufficient because activation
			 * records are propagated to the parent on nested transaction commit, and
			 * dropped when the (nested) action aborts. Thus, an object remains active as
			 * long as a single ActivationRecord is being used, and we don't need to create
			 * a new record for each transaction in the same hierarchy. Once activated, the
			 * object remains active until the action commits or aborts (at which time it
			 * may be passivated, and then reactivated later by the creation of a new
			 * ActivationRecord.)
			 */
		...
		if (forceAR || (currentStatus == ObjectStatus.PASSIVE) || (currentStatus == ObjectStatus.PASSIVE_NEW)) {
			/*
			 * If object is recoverable only, then no need to set up the object store.
			 */
			...
			if (currentStatus == ObjectStatus.PASSIVE) {
				/*
				 * If the object is shared between different processes, then we must load the
				 * state each time a top-level action accesses it. Otherwise we can continue to
				 * use the last state as in Dharma/ArjunaII.
				 */
			...
			/*
			 * Create ActivationRecord if status changed Passive->Active or if object is a
			 * new persistent object.
			 */

			if (forceAR || ((currentStatus == ObjectStatus.ACTIVE) || (currentStatus == ObjectStatus.PASSIVE_NEW))
					&& (action != null)) {
				...
				if ((arStatus = action.add(ar)) != AddOutcome.AR_ADDED) {
					...
					/*
					 * We never reset activated, so we can optimise state loading/unloading in the
					 * case of SINGLE object model
					 */
	...
	public synchronized boolean deactivate(String rootName, boolean commit) {
		if (tsLogger.logger.isTraceEnabled()) {
			tsLogger.logger.trace("StateManager::deactivate(" + ((rootName != null) ? rootName : "null") + ", " + commit
					+ ") for object-id " + objectUid);
		}
		...
		if ((currentlyActivated && (myType == ObjectType.ANDPERSISTENT)) || loadObjectState()) {
			...
			if ((currentStatus == ObjectStatus.ACTIVE_NEW) || (currentStatus == ObjectStatus.ACTIVE)) {
				...
				/*
				 * Call save_state again to possibly get a persistent representation of the
				 * object.
				 */
				...
				/*
				 * Not needed any more because activation record does this when all actions are
				 * forgotten. if (result) currentStatus = ObjectStatus.PASSIVE;
				 */
			}
		} else {
			result = true;
	 ...
	 * variable is checked in the modified operation so that no recovery information
	 * is created if myType is set to NEITHER.
	 */
	...
	/**
	 * The object's type. Derived classes should override this to reflect their type
	 * structure. Typically this string is used for locating the object state in an
	 * object store, and reflects the hierarchy structure of the object.
	 */
	...
	/**
	 * @return the root of the object store this instance will use if it has to save
	 *         the state of the object to storage.
	 */
	...
	/**
	 * @return the object store this instance will used if it has to save the state
	 *         of the object to storage.
	 */
	...
	protected void packHeader(OutputObjectState os, Header hdr) throws IOException {
		/*
		 * If there is a transaction present than pack the process Uid of this JVM and
		 * the tx id. Otherwise pack a null Uid.
		 */
		...
		try {
			...
			/*
			 * Only pack something if there is a transaction. Otherwise the application is
			 * driving this object manually, and all bets are off!
			 */
			...
			if (tsLogger.logger.isTraceEnabled()) {
				tsLogger.logger
						.trace("StateManager.packHeader for object-id " + get_uid() + " birth-date " + birthDate);
			}
	...
	protected void unpackHeader(InputObjectState os, Header hdr) throws IOException {
		try {
			...
			if (tsLogger.logger.isTraceEnabled()) {
				tsLogger.logger
						.trace("StateManager.unpackHeader for object-id " + get_uid() + " birth-date " + birthDate);
			}
	 ...
	 * recreating an existing object. The object type is set to 'ANDPERSISTENT' this
	 * is equivalent to invoking persist in the object constructor.
	 */
	...
	protected synchronized boolean modified() {
		...
		if (action != null) {
			/*
			 * Check if this is the first call to modified in this action. BasicList insert
	 ...
	 * ANDPERSISTENT. No changes are made unless the status of the object is ACTIVE,
	 * so it is not possible to change the type of the object if it has been
	 * modified.
	 */
	...
	protected final synchronized void cleanup(boolean fromTerminate) {
		...
		synchronized (mutex) {
			...
			if (usingActions.size() > 0) {
				...
				while (e.hasMoreElements()) {
					...
					if (action != null) {
						/*
						 * Pop actions off using list. Don't check if action is running below so that
		...
		/*
		 * Here the object must be either RECOVERABLE or PERSISTENT. Whether or not an
		 * action exists we still need to reset the object status to avoid possible
		 * later confusion. What it gets set to is not important really as long as it
		 * gets changed from ACTIVE_NEW which might cause any running action to abort.
		 */
	...
	protected synchronized void setupStore(String rootName) {
		setupStore(rootName, arjPropertyManager.getObjectStoreEnvironmentBean().getObjectStoreType());
	}
	...
	protected final boolean loadObjectState() {
		...
		/*
		 * MULTIPLE object model requires loading of state every time, even if we are
		 * RECOVERABLE - we use the volatile store.
		 */

		if (!load) {
			/*
			 * Must be SINGLE object model. So, is this the first time? If so, load state.
			 */
	...
	/**
	 * Remove action from list of using actions. If the action list empties reset
	 * state to PASSIVE. The second param tells why the action should be forgotten.
	...
	protected final synchronized boolean forgetAction(BasicAction action, boolean committed, int recordType) {
		if (tsLogger.logger.isTraceEnabled()) {
			tsLogger.logger.trace("StateManager::forgetAction(" + ((action != null) ? action.get_uid() : Uid.nullUid())
					+ ")" + " for object-id " + objectUid);
		}
			...
			synchronized (mutex) {
				if (usingActions != null) {
					...
					if (usingActions.size() == 0) {
						if (committed) {
							if ((myType == ObjectType.RECOVERABLE) && (objectModel == ObjectModel.SINGLE)
									|| (action.typeOfAction() == ActionType.NESTED)) {
	...
	protected final synchronized boolean rememberAction(BasicAction action, int recordType, int state) {
		if (tsLogger.logger.isTraceEnabled()) {
			tsLogger.logger.trace("StateManager::rememberAction("
					+ ((action != null) ? action.get_uid() : Uid.nullUid()) + ")" + " for object-id " + objectUid);
		}
	...
	/**
	 * @return <code>true</code> if the object was locked, <code>false</code> if the
	 *         attempt would cause the thread to block.
	 * @since JTS 2.1.
	 */
	...
	/*
	 * Delay creating these lists until we really need them. Some transactions may
	 * start and end without adding any participants or being involved with multiple
	 * threads. Some classes (e.g., AbstractRecords) that inherit from StateManager
	 * may never need these lists either.
	 */
