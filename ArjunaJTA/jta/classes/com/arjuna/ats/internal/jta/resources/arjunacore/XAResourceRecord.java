...
public class XAResourceRecord extends AbstractRecord implements ExceptionDeferrer {
	...
	/**
	 * The params represent specific parameters we need to recreate the connection
	 * to the database in the event of a failure. If they're not set then recovery
	...
	{
    	try(Scope scope = new ScopeBuilder(SpanName.GLOBAL_PREPARE)
    			.tag(TagNames.XID, this._tranID).start()) {
			if (jtaLogger.logger.isTraceEnabled()) {
		...
		try (Scope scope = new ScopeBuilder(SpanName.GLOBAL_COMMIT).start()) {
			...
			if (_tranID == null) {
				...
				if (_theXAResource != null) {
					...
					/*
					 * No need for end call here since we can only get to this point by going
					 * through prepare.
					 */
					...
					try {
						...
						if (notAProblem(e1, true)) {
							...
							case XAException.XAER_RMFAIL:
								...
								/*
								 * Could do timeout retry here, but that could cause other resources in the list
								 * to go down the heuristic path (some are far too keen to do this). Fail and
								 * let recovery retry. Meanwhile the coordinator will continue to commit the
	...
	/**
	 * For commit_one_phase we can do whatever we want since the transaction outcome
	 * is whatever we want. Therefore, we do not need to save any additional
	 * recoverable state, such as a reference to the transaction coordinator, since
	...
	public int topLevelOnePhaseCommit() {
		...
		if (_tranID == null) {
			...
			if (_theXAResource != null) {
				...
				try {
					/*
					 * TODO in Oracle the end is not needed. Is this common across other RMs?
					 */

					endAssociation(XAResource.TMSUCCESS, TxInfo.NOT_ASSOCIATED);
				} catch (XAException e1) {
					/*
					 * Now it's not legal to return a heuristic from end, but apparently Oracle does
					 * (http://jira.jboss.com/jira/browse/JBTM-343) Since this is 1PC we can call
					 * forget: the outcome of the transaction is the outcome of the participant.
					 */
				...
				try {
					/*
					 * Not strictly necessary since calling commit will do the rollback if end
					 * failed as above.
					 */
					...
					/*
					 * XA_HEURHAZ, XA_HEURCOM, XA_HEURRB, XA_HEURMIX, XAER_RMERR, XAER_RMFAIL,
					 * XAER_NOTA, XAER_INVAL, or XAER_PROTO. XA_RB*
					 */
					...
					// The alternative implication is that the RM timed out the branch between the
					// end above and the completion call, if we do make a change to assume that
					// scenario
					// it is possible we could break existing deployments so changes should be
					// considered and potentially configurable
					case XAException.XAER_INVAL: // resource manager failed, did it rollback?
						_heuristic = TwoPhaseOutcome.HEURISTIC_HAZARD;
						return TwoPhaseOutcome.HEURISTIC_HAZARD;
	...
	public boolean forgetHeuristic() {
		...
		// remove the connection regardless of whether or not the forget operation
		// failed
		removeConnection();
	...
	protected int recover() {
		...
		if (_committed) {
			/*
			 * A previous commit attempt failed, but we know the intention was to commit. So
			 * let's try again.
			 */
	...
	public boolean save_state(OutputObjectState os, int t) {
		...
		try {
			...
			/*
			 * Since we don't know what type of Xid we are using, leave it up to XID to
			 * pack.
			 */
			...
			jtaLogger.i18NLogger.warn_resources_arjunacore_savestateerror(_theXAResource.toString(),
	...
	public boolean restore_state(InputObjectState os, int t) {
		...
		try {
			...
			if (os.unpackInt() == RecoverableXAConnection.OBJECT_RECOVERY) {
					...
					try {
						...
						if (!deserialized) {
							try {
								...
								// JBTM-2550 if we fail to deserialize the object, we treat it as haveXAResource
								// is false
				...
				if (!haveXAResource) {
					...
					if (_theXAResource == null) {
						...
						/*
						 * Don't prevent tx from activating because there may be other participants that
						 * can still recover. Plus, we will try to get a new XAResource later for this
						 * instance.
						 */
			...
			jtaLogger.i18NLogger.warn_resources_arjunacore_restorestateerror(_theXAResource.toString(),
					XAHelper.xidToString(_tranID), e);
			...
			/*
			 * If we're here then we've restored enough to print data on this instance.
			 */
	...
	/**
	 * This routine finds the new XAResource for the transaction that used the old
	 * resource we couldn't serialize. It does this by looking up the
	 * XARecoveryModule in the recovery manager and asking it for the XAResource.
	 * The recovery manager will then look through its list of registered
	 * XARecoveryResource implementations for the appropriate instance. If the
	 * XARecoveryModule hasn't been initialised yet then this routine will fail, but
	 * on the next scan it should work.
	 */
	...
	private final void removeConnection() {
		/*
		 * Should only be called once. Remove the connection so that user can reuse the
		 * driver as though it were fresh (e.g., can do read only optimisation).
		 */
	...
	/*
	 * Ask the transaction whether or not this XAResource is still associated with
	 * the thread, i.e., has end already been called on it?
	 */
	...
	private static final XAResourceRecordWrappingPlugin _xaResourceRecordWrappingPlugin = jtaPropertyManager
			.getJTAEnvironmentBean().getXAResourceRecordWrappingPlugin();

	private static final boolean _rollbackOptimization = jtaPropertyManager.getJTAEnvironmentBean()
			.isXaRollbackOptimization();

	/*
	 * WARNING: USE WITH EXTEREME CARE!!
	 *
	 * This assumes that if there is no XAResource that can deal with an Xid after
	 * recovery, then we failed after successfully committing the transaction but
	 * before updating the log. In which case we just need to ignore this resource
	 * and remove the entry from the log.
	 *
	 * BUT if not all XAResourceRecovery instances are correctly implemented (or
	 * present) we may end up removing participants that have not been dealt with.
	 * Hence USE WITH EXTREME CARE!!
	 */
